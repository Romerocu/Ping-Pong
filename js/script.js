...
function render() {
    ...
    if (ai.score < 20 && user.score < 20){
      if(ai.score == 0){
        drawNet();
      // draw user score
        drawScore(canvas.width / 4, canvas.height / 6, user.score, 'green');
        // draw ai score
        drawScore(3 * canvas.width / 4, canvas.height / 6, ai.score, 'red');
        // draw user paddle
      ...
      // draw user score
        ...
        drawBall(ball.x, ball.y, ball.radius, ball.color);
        if (user.scored == 1){
          drawWin(500,400,"User got a point.");
        } else if (ai.scored==1){
          drawWin(500,400,"Ai got a point.")
        }
      }
    } else {
      if (ai.score >= 20){
        drawWin(500,400,"Ai Wins.")
      } else if (user.score >= 20) {
        drawWin(500,400,"Player Wins.")
      }
    }
}
